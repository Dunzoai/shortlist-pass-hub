...
export default function AffiliatesPage() {
  ...
  useEffect(() => {
    async function fetchAffiliates() {
      ...
      // Fetch affiliates with their clients
      const { data: affiliatesData } = await supabase
        .from('affiliates')
        .select('*, clients(*)')
        .order('name')

      // Fetch all active client services to calculate revenue
      ...
      const affiliatesWithStats = ((affiliatesData as (Affiliate & { clients: Client[] })[]) ?? []).map(
        (affiliate) => {
          const clientIds = affiliate.clients.map((c) => c.id)

          // Sum monthly revenue from this affiliate's clients
          ...
          // Calculate affiliate earnings based on their payment terms
          let totalEarnings = 0
          if (affiliate.payment_type === 'percentage' && affiliate.payment_amount) {
            ...
            if (affiliate.payment_frequency === 'monthly') {
              totalEarnings = affiliate.payment_amount * affiliate.clients.length
            } else {
              // One-time - we'd need to track if paid, for now show potential
              totalEarnings = affiliate.payment_amount * affiliate.clients.length
            }
          }
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        ...
        <Link
          ...
          className="px-4 py-2 bg-[#2E8B57] hover:bg-[#25724a] text-white font-medium rounded-lg transition-colors"
        >
          Add Affiliate
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          <>
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-48 bg-[#333333] rounded-lg animate-pulse"></div>
            ))}
          </>
        ) : affiliates.length === 0 ? (
          <div className="col-span-full bg-[#333333] rounded-lg p-8 text-center text-gray-400">
            No affiliates yet.{' '}
            <Link href="/admin/affiliates/new" className="text-[#2E8B57] hover:underline">
          ...
          affiliates.map((affiliate) => (
            <Link
              ...
              className="bg-[#333333] rounded-lg p-6 hover:ring-2 hover:ring-[#2E8B57] transition-all"
            >
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-200">
                    {affiliate.payment_type === 'percentage'
                      ? `${affiliate.payment_amount}%`
                      : affiliate.payment_type === 'flat'
                      ? `$${affiliate.payment_amount}`
                      : '-'}
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-200">
                    ${affiliate.totalMonthlyRevenue.toLocaleString()}/mo
                  </span>
                </div>
