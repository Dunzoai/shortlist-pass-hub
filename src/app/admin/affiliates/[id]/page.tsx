...
export default function EditAffiliatePage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      // Fetch clients referred by this affiliate
      ...
      // Fetch active services for these clients
      ...
      if (clientIds.length > 0) {
        ...
        const clientsWithRevenue = ((clientsData as Client[]) ?? []).map((client) => {
          ...
          const monthlyRevenue = clientServices.reduce(
  ...
  const handleDelete = async () => {
    if (
      !confirm(
        'Are you sure you want to delete this affiliate? Their referred clients will no longer be associated with them.'
      )
    ) {
      return
    }
  ...
  if (!affiliate) {
    return (
      <div className="p-8">
        ...
        <Link href="/admin/affiliates" className="text-[#2E8B57] hover:underline">
          Back to Affiliates
        </Link>
      </div>
    )
